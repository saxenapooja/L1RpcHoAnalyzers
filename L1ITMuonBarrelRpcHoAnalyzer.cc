...
// Class:      L1ITMuonBarrelRpcHoAnalyzer
// 
/**\class L1ITMuonBarrelRpcHoAnalyzer L1ITMuonBarrelRpcHoAnalyzer.cc L1Trigger/L1IntegratedMuonTrigger/plugins/L1ITMuonBarrelRpcHoAnalyzer.cc
...
class L1ITMuonBarrelRpcHoAnalyzer : public edm::EDAnalyzer {

public:

  //constructor
  explicit L1ITMuonBarrelRpcHoAnalyzer(const edm::ParameterSet&);

  //destructor
  ~L1ITMuonBarrelRpcHoAnalyzer();
...
private:
  ...
  virtual void SetLabelsName(TAxis &axe);
  // ----------member data ---------------------------
public:
  UInt_t event;
  UInt_t run;
  ...
  bool   tkEvtB  = false;
  UInt_t nHighQDt       = 0;
  UInt_t nCorrectBxDt   = 0;
  UInt_t nCorrectBxHO   = 0;
  UInt_t nRpcColExist   = 0;
  UInt_t nHoColExist    = 0;
  UInt_t nRpcHoPairExist =0;
  UInt_t nRpcHoAlgo      =0;
};
...
L1ITMuonBarrelRpcHoAnalyzer::L1ITMuonBarrelRpcHoAnalyzer(const edm::ParameterSet& iConfig)
...
L1ITMuonBarrelRpcHoAnalyzer::~L1ITMuonBarrelRpcHoAnalyzer()
{
...
void L1ITMuonBarrelRpcHoAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for ( ; st != stend; ++st ) {
    ...
    if(doDebug)    std::cout<<"Event, mbltCollection  is :"<< iEvent.id().event()<<", "<< dtListSize << std::endl;
    ...
    for( size_t idxDt = 0; idxDt < cSize; ++idxDt )	{
      ...
      if(doDebug) std::cout<<"DT-algo::sizes are, "<<rpcInMatchSize<<", "<< rpcOutMatchSize<< std::endl;

      // RPC collection should not be empty
    ...
    nCorrectBxDt++;
      ...
      // HO collection should not be zero
      ...
      if(doDebug) std::cout<<"DT-algo:: HO sizes are, "<<hoMatchSize << std::endl;
      nHoColExist++;
	...
	bendingAngle = -999.0;
      ...
      //      std::cout<<"HO VAUES IS ####################################"<< HOGEta <<" ,"<< HOGPhi << std::endl;
    ...
    //    if(tkEvtA && tkEvtB) break; //changed
  }// for ( ; st != stend; ++st )

  std::cout<<"Final counters : "<< nHighQDt <<", "<< nRpcColExist <<", " << nCorrectBxDt  << ", "<< nHoColExist << std::endl;  
...
void 
  ...
  //counters
  tree->Branch("nHighQDt"     , &nHighQDt,"nHighQDt/I");
  tree->Branch("nRpcColExist" , &nRpcColExist,"nRpcColExist/I");
  tree->Branch("nCorrectBxDt" , &nCorrectBxDt,"nCorrectBxDt/I");
  tree->Branch("nHoColExist"  , &nHoColExist,"nHoColExist/I");
  tree->Branch("nCorrectBxHO" , &nCorrectBxHO,"nCorrectBxHO/I");
  tree->Branch("nRpcHoPairExist" , &nRpcHoPairExist,"nRpcHoPairExist/I");
  tree->Branch("nRpcHoAlgo" , &nRpcHoAlgo,"nRpcHoAlgo/I");
}
 
// ------------ method called once each job just after ending the event loop  ------------
void 
L1ITMuonBarrelRpcHoAnalyzer::endJob() 
{
}
...
/*
void 
L1ITMuonBarrelRpcHoAnalyzer::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
L1ITMuonBarrelRpcHoAnalyzer::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
L1ITMuonBarrelRpcHoAnalyzer::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
L1ITMuonBarrelRpcHoAnalyzer::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
L1ITMuonBarrelRpcHoAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void L1ITMuonBarrelRpcHoAnalyzer::SetLabelsName(TAxis &axe)
{
  axe.CenterTitle();
  axe.SetBinLabel(1, "nHighQDt");
  axe.SetBinLabel(2, "nRpcCol");
  axe.SetBinLabel(3, "nCorDTBx");
  axe.SetBinLabel(4, "nHoCol");
  axe.SetBinLabel(5, "nRpcHoPair");
  axe.SetBinLabel(6, "nRpcHoAlgo");
  axe.LabelsOption("h");  // FIXME: Is "a" thread safe? (Paint and Fill?)
}


//define this as a plug-in
DEFINE_FWK_MODULE(L1ITMuonBarrelRpcHoAnalyzer);
